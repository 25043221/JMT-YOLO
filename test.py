...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        # Compute loss
        if compute_loss:
            loss += compute_loss([x.float() for x in det_out], targets)[1][:3]  # box, obj, cls
        ...
        if augment:
            ...
            out = non_max_suppression(nms_out, conf_thres, iou_thres, labels=lb, multi_label=True, agnostic=single_cls)
