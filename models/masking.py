...
class Masking(nn.Module):
    ...
    def forward(self, img: Tensor):
        ...
        mean = torch.tensor(self.augmentation_params['mean'], dtype=torch.float, device=img.device).view(1, 3, 1, 1)
        std = torch.tensor(self.augmentation_params['std'], dtype=torch.float, device=img.device).view(1, 3, 1, 1)
