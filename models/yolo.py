...
class SegMaskPSP(nn.Module):
    def __init__(self, n_segcls=9, n=1, c_hid=256, shortcut=False, ch=()):
        ...
        self.out = nn.Sequential(
                                DenseRFB2(c_hid*3, c_hid, d=[2, 3], map_reduce=6),
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5l_city_seg.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1 and x % i not in save)
        layers.append(m_)
        if i == 0:
            ch = []
        ch.append(c2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov5l_city_seg.yaml', help='model.yaml')
