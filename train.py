...
from utils.MICloss import KLDivergenceLossForOutputs
from utils.seg_loss import CrossEntropySegmentationLoss
...
from utils.datasets import create_dataloader, create_segmentation_dataloader
from utils.datasets_single import create_dataloader_single
from utils.google_utils import attempt_download
from utils.loss import ComputeLoss, dice_loss_multi_class
...
from models.masking import Masking  # Import Masking for masked image consistency
...
def train(hyp, opt, device):
    ...
    train_path_source_seg_real = data_dict['train_source_seg_real']
    train_path_source_seg_fake = data_dict['train_source_seg_fake']
    train_path_target_seg_real = data_dict['train_target_seg_real']
    train_path_target_seg_fake = data_dict['train_target_seg_fake']
    ...
    if pretrained:
        ...
        if 'optimizer' in ckpt and ckpt['optimizer'] is not None:
            ...
            print("No optimizer state found in checkpoint, initializing optimizer with default settings.")
    ...
    dataloader_sr_seg, dataset_sr_seg = create_segmentation_dataloader(train_path_source_seg_real, train_path_source_seg_fake, imgsz,
                                                                                  batch_size // WORLD_SIZE,
                                                                                  gs, single_cls, hyp=hyp, augment=False, cache=False,
                                                                                  rect=False, rank=RANK, workers=workers,
                                                                                  image_weights=opt.image_weights, quad=opt.quad,
                                                                                  prefix=colorstr('train_source_real_seg_fake: '))
    dataloader_tr_seg, dataset_tr_seg = create_segmentation_dataloader(train_path_target_seg_real, train_path_target_seg_fake, imgsz,
                                                  batch_size // WORLD_SIZE,
                                                  gs, single_cls, hyp=hyp, augment=False, cache=False,
                                                  rect=False, rank=RANK, workers=workers,
                                                  image_weights=opt.image_weights, quad=opt.quad,
                                                  prefix=colorstr('train_target_real_seg_fake: '))
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            # cf = torch.bincount(c.long(), minlength=nc)best_fitness + 1.  # frequency
    ...
    # MIC: Initialize Masking and Teacher Models
    masking = Masking(
        block_size=opt.mask_block_size,
        ratio=opt.mask_ratio,
        color_jitter_s=opt.mask_color_jitter_s,
        color_jitter_p=opt.mask_color_jitter_p,
        blur=opt.mask_blur,
    ...
    for epoch in range(start_epoch, epochs):  # start epoch ------------------------------------------------------------
        ...
        if RANK != -1:  # load dats sequentially in UMT
            ...
            dataloader_tr_seg.sampler.set_epoch(epoch)
            dataloader_sr_seg.sampler.set_epoch(epoch)
        ...
        data_iter_sr_seg = iter(dataloader_sr_seg)
        data_iter_tr_seg = iter(dataloader_tr_seg)
        ...
        for i, ind in pbar:
            ...
            # for segmentation data, source and target
            try:
                imgs_sr_seg, imgs_sf_seg, targets_sr_seg, paths_sr_seg, paths_sf_seg, _ = next(data_iter_sr_seg)
            except StopIteration:
                data_iter_sr_seg = iter(dataloader_sr_seg)
                imgs_sr_seg, imgs_sf_seg, targets_sr_seg, paths_sr_seg, paths_sf_seg, _ = next(data_iter_sr_seg)
            imgs_sr_seg = imgs_sr_seg.to(device, non_blocking=True).float() / 255.0
            imgs_sf_seg = imgs_sf_seg.to(device, non_blocking=True).float() / 255.0

            try:
                imgs_tr_seg, imgs_tf_seg, targets_tr_seg, paths_tr_seg, paths_tf_seg, _ = next(data_iter_tr_seg)
            except StopIteration:
                data_iter_tr_seg = iter(dataloader_tr_seg)
                imgs_tr_seg, imgs_tf_seg, targets_tr_seg, paths_tr_seg, paths_tf_seg, _ = next(data_iter_tr_seg)
            imgs_tr_seg = imgs_tr_seg.to(device, non_blocking=True).float() / 255.0
            imgs_tf_seg = imgs_tf_seg.to(device, non_blocking=True).float() / 255.0
            ...
            with amp.autocast(enabled=cuda):
                ...
                # save_image(imgs_sr, "original_img.png")
                pred_sr = model_student(imgs_sr)  # forward
                det_pred_sr = pred_sr[0]
                seg_pred_sr = pred_sr[1]

                loss_sr, loss_items_sr = compute_loss(det_pred_sr, targets_sr.to(device))  # loss scaled by batch_size
                loss_seg_sr = segloss_fn(seg_pred_sr, targets_sr_seg.to(device).long())

                # [branch 2] for model_student, with using labels
                pred_sf = model_student(imgs_sf)  # forward
                det_pred_sf = pred_sf[0]
                seg_pred_sf = pred_sf[1]
                loss_sf, loss_items_sf = compute_loss(det_pred_sf, targets_sr.to(device))  # loss scaled by batch_size
                loss_seg_sf = segloss_fn(seg_pred_sf, targets_sr_seg.to(device).long())
                ...
                det_pred_tr = pred_tr[0]
                seg_pred_tr = pred_tr[1]
                ...
                loss_distillation, loss_items_distillation = compute_loss(det_pred_tr, pred_labels.to(
                    device))  # loss scaled by batch_size

                # # [branch 5] for model_student, without using labels, use masking
                ...
                # #使用多分类 Dice 损失
                # # mask_loss = dice_loss_multi_class(probs_teacher, probs_student)
                ...
                # loss = loss_sr + loss_sf + loss_seg_sr + loss_seg_sf + loss_distillation * lambda_weight + mask_loss
                loss_items = loss_items_sr + loss_items_sf + loss_items_distillation * lambda_weight
                # Add consistency loss if enabled
                if opt.consistency_loss:
                    # loss = loss + loss_cons
                    # loss = 0.6 * (loss + loss_cons) + 0.4 * mask_loss
                    loss = 0.6*(loss + loss_cons) + 0.4*(loss_seg_sr + loss_seg_sf + mask_loss)
                    loss_items[3] += loss_cons.detach()[0]  # Update total loss in loss_items
                    loss_items = torch.cat((loss_items, loss_cons.detach()), 0)  # Concatenate consistency loss
            ...
            # Print
            if RANK in [-1, 0]:
                # print("mloss shape:", mloss.shape)  #5
                # print("loss_items shape:", loss_items.shape)  #13
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5l_city_seg.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/yamls_sda/cityscapes_csfoggy_VOC.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--conf_thres', type=float, default=0.8, help='Confidence threshold for pseudo label in UMT')
    ...
    # parser.add_argument('--lambda_masking_weight', type=float, default=1.0, help='mic loss')
    ...
    # Add arguments for MIC mechanism
    parser.add_argument("--mask_block_size", type=int, default=64, help="Size of blocks for masking")
    parser.add_argument("--mask_ratio", type=float, default=0.6, help="Ratio of masking")
    parser.add_argument("--mask_color_jitter_s", type=float, default=0.2, help="Strength of color jitter")
    parser.add_argument("--mask_color_jitter_p", type=float, default=0.2, help="Probability of color jitter")
    parser.add_argument("--mask_blur", action='store_true', default=True, help="Apply blur during masking")
